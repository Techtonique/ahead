...
```{r fig.width=7.5}
(obj <- suppressWarnings(ahead::glmthetaf(USAccDeaths, h=25L, fit_func=stats::lsfit, attention = TRUE)))
plot(obj)
```
...
```{r fig.width=7.5}
(obj <- suppressWarnings(ahead::glmthetaf(USAccDeaths, h=25L, fit_func=glmnet::glmnet, attention = TRUE)))
plot(obj)
```

```{r fig.width=7.5}
(obj <- suppressWarnings(ahead::glmthetaf(USAccDeaths, h=25L, fit_func=glmnet::cv.glmnet, attention = TRUE)))
plot(obj)
```
