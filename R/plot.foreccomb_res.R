...
plot.foreccomb_res <- function(x, which=1,...) {
    ...
    if (is.null(forec) & is.null(newobs_vector)) {
        ...
        if (is.null(newobs_vector) == FALSE) {
            ...
            p <- ggplot2::ggplot(data = pl, ggplot2::aes(x = Index)) + ggplot2::geom_line(aes(y = Actual, colour = "ACTUAL"), size = 0.5) + ggplot2::geom_line(aes(y = c(Combined_Fit), colour = "COMBINED (FIT)"),
