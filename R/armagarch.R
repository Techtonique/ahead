...
armagarchf <- function(y,
  ...
  if(!is.null(B)){
    ...
    ans$x <- y
    ans$level <- level
    ans$model <- list(obj_arma=obj_arma, obj_garch=obj_garch)
    preds_main <- predict(obj_arma, n.ahead=h, se.fit=FALSE)
    preds_vol <- fGarch::predict(obj_garch, n.ahead=h, conf=level/100, plot=TRUE); dev.off();
