...
#' @param conformal A boolean; either use conformal prediction or not
...
mlarchf <- function(y,
                    ...
                    type_sim_conformalize = c("surrogate", "block-bootstrap", 
                                              "bootstrap", "kde", "fitdistr"),
                    ml_method = NULL,
                    level = 95,
                    B = 250L,
                    ml = TRUE,
                    ...
                    ...)
{
  ...
  if (conformal)
  {
    if (ml == TRUE)
    {
      if (!is.null(ml_method)) {
        fit_func <- function(x, y, method = ml_method, ...) {
          ...
          if (n_clusters > 0) {
            ...
            clustering_obj <- cclust::cclust(
              cbind(as.matrix(scaler_x$X), 0),
              centers = n_clusters,
        ...
        predict_func <- function(obj, newx) {
          ...
          if (n_clusters > 0) {
            ...
            newx_scaled <- misc::scale_matrix(
              as.matrix(newx),
              X_mean = obj$scaler_x$X_mean,
              X_sd = obj$scaler_x$X_sd
            )$X
            # Predict clusters for new data
            new_clusters <- predict(obj$clustering_obj, cbind(newx_scaled, 0))$cluster
            # Combine with newx
      ...
      f <- as.numeric(fit_mean$mean) + matrix(fit_z$sims, ncol = B) * sqrt(matrix(exp(fit_sigma$sims), ncol =
                                                                                    B))
      mean_f <- rowMeans(f)
      alpha <- 1 - level / 100
      lower_bound <- apply(f, 1, function(x)
        quantile(x, probs = alpha / 2))
      upper_bound <- apply(f, 1, function(x)
      ...
      out$sims <- ts(f, start = start_preds, frequency = freq_x)
      if (!is.null(ml_method))
      {
      ...
      out$lower <- ts(lower_bound, start = start_preds, frequency = freq_x)
      out$upper <- ts(upper_bound, start = start_preds, frequency = freq_x)
    ...
    } else
    {
      ...
      fit_func_sigma <- function(FUN = stat_model,
                                 y = y,
                                 h = h,
                                 level = level,
                                 ...)
      {
        ahead::genericforecast(
          FUN = FUN,
          y = y,
          h = h,
          level = level,
